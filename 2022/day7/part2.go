...
func solveNoSpaceLeftOnDevice() (int, error) {
	...
	freeSize := totalFSSize - root.Size
	if freeSize >= minFreeSizeRequired {
		return 0, fmt.Errorf("nothing to delete, free size is %d", freeSize)
	}

	needToFree := minFreeSizeRequired - freeSize

	minSize := getMinSizeRequiredToDelete(root, needToFree)
	if minSize == -1 {
		return 0, errors.New("min size of dir required to delete not found")
	}

	return minSize, nil
}

func getMinSizeRequiredToDelete(root *fsDir, minSize int) int {
	min := -1
	...
	for _, child := range root.Children {
		childMin := getMinSizeRequiredToDelete(child, minSize)
		if childMin != -1 && (min == -1 || childMin < min) {
			min = childMin
		}
	}
